...
- **Статус:** Accepted (реализован — 2026-07-06) · **дополнен ADR-0103/0104/0105** (2026-07-07):
  правило выбора размера — non-inferiority (`refine_tol`, ADR-0103) вместо двусторонней «неотличимости»
  при низкой мощности; пол descent — `max(min_features, seq_min_features, mass_floor(refine_min_mass))`
  (ADR-0104/0105), где `min_features` теперь учтён каскадом
- **Дата:** 2026-07-06
...
- **Same-OOF residual (ADR-0085 §5) теперь на дефолтном ранкер-пути:** якорь — argmax на тех же
  OOF-фолдах, что тестирует band, поэтому выбор размера каскада несёт лёгкий residual-оптимизм.
  До 1.1 это касалось только обёртки `sequential`; с `refine=True` по умолчанию распространяется на
  ранкеры. Направление консервативное (band ⇒ «оставить больше», не over-prune); независимый-OOF —
  Day-2 воркстрим (тот же, что пере-ранжирование §Рассмотренные-2).
