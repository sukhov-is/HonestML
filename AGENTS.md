...
- For `.ipynb`, edit the notebook JSON in place with `apply_patch`, keeping cell structure, ids and recorded outputs intact.
- Delegate independent subtasks through the provider's available subagent mechanism — broad "how/where/what" codebase searches, isolated data checks, and documentation lookups; intervene if a subagent goes off track or is missing relevant context. A subagent starts with none of your context: give it explicit paths, the contract to judge against, and what its answer unblocks. For an adversarial check hand over the artifact alone — a reviewer who sees your conclusion agrees with it.
- In a loop round, fan out in one step and collect the results together, and never leave a subagent running in the background or wait by polling — a leftover task wakes the finished round and every poll turn re-pays the whole context. In an interactive session background subagents are fine: keep working while they run.
- Use the available current-documentation tools when implementation details for external libraries are uncertain or up-to-date documentation/examples are needed.
</tool_usage>
