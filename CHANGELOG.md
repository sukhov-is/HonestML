...
### Breaking changes
- Публичный `RunContext` учитывает длительности через события стадий: повторные `timed_stage` и `record_stage_time` суммируются, а `total_time(key)` возвращает сумму эксклюзивного измеренного времени и исторического времени кеша. Начальное содержимое `timings` и прямые изменения этого словаря не создают событий для `total_time`.
- Для переноса исторических длительностей используйте `record_stage_time(key, stage, elapsed)`; для текущих измерений — `timed_stage(key, stage)`. `timings` служит накопленным представлением длительностей, а `cost_report()` разделяет полное время запуска, вложенные стадии и кеш. Формат сохранённых моделей остаётся `ARTIFACT_VERSION=1`.

### Added
- Условный прогноз завершения FS/HPO до выбора семейства: два ограниченных измерения, отдельный потолок fit, явные допущения и неопределённая стоимость при недостатке данных.
- Замороженный парный протокол внешней проверки качества с привязкой предсказаний, меток, scorer и контракта по SHA256; статистический вывод отделён от проверки независимости данных.
- Подтверждение максимум двух семейств на втором ограниченном ресурсе, диагностика ES/классов и выбор по стоимости при подтверждённой non-inferiority в явном допуске DEV-метрики.
- Ограничение строк и фактических fit выбранного FS-рецепта; возврат к широкому контролю при исчерпании FS-бюджета или неосуществимом классовом лимите.
- Подготовленные вклады аддитивных метрик для bootstrap и локальные инструменты DEV-абляций с одинаковыми ресурсами.
- Ограниченный поиск через `SearchConfig`: предварительный выбор семейства и многоэтапного FS-рецепта, отдельный широкий контроль, резерв завершения и отчёт решений.
- Полная стоимость `fit`, повторные и вложенные стадии, фактические fit/trial/fold, tree budget и доступные iterations, выборочное измерение RSS.
- Кеш повторных FS-оценок, сохранение подготовки перед HPO и возобновление истории trials с проверкой контекста.
- Отдельный протокол сравнения времени и качества в `benchmarks/training_cost.py` и руководство `docs/training-performance.md`.

### Changed
- Default `SearchConfig.confirmation_rows=65536`: общий лимит fit/ES/test по folds одной семьи на этапе подтверждения, отдельно от начальных проб. `max_features` ограничивает FS, а не ширину модельных проб; `estimated_cv_refit_s` описывает компонент CV/refit; `completion_cost_forecast` добавляет условную стоимость поддержанных FS/HPO-процедур и не является прогнозом полного wall time.
- Одно явно заданное семейство `models` и одна FS-стратегия без `compare` пропускают сравнение семейств/рецептов, сохраняя ресурсные ограничения выбранного FS.
- Подготовленные ROC/PR bootstrap scorers для поддержанных входов sklearn 1.7.2 и ограниченный кеш встроенного scorer ансамбля сокращают повторные расчёты без изменения статистических критериев и принятия ансамбля. Пользовательские и изменённые scorers сохраняют общий путь, включая аддитивный bootstrap.
- Внутренние IID HPO folds выделяют ES внутри train. Scope кеша учитывает вычислительную семантику подготовки и завершения поиска.
- Явное число деревьев HPO ограничивает ES-fit. DEV/full refit и участники ансамбля сохраняют бюджет деревьев выбранной фабрики: default, явные параметры или результат HPO. Медиана фактических CV iterations используется только для диагностики и прогноза стоимости; число деревьев не выбирается по внешнему holdout.
- Невалидные HPO trials имеют отдельный статус и не создают настроенную модель; несовместимый или незавершённый кеш не переиспользуется.
- Формат candidate/stage cache — 3; несовместимые записи пересчитываются. Формат HPO checkpoint — 2. Прогноз стоимости отдельно учитывает CV и настроенный бюджет refit.

### Fixed
- Руководства и проза восьми ноутбуков описывают сохранённые контрольные прогоны, широкий контроль и границы независимости оценки. Публичная сводка сохраняет незавершённый статус исходного Home fast и отделяет отдельные final-прогоны от основной серии.
- Manifest и run fingerprint отражают фактически разрешённый каталог моделей, включая явно заданное единственное семейство и пользовательские components.
- DEV-пробы `benchmarks/search_diagnostics.py` используют `full.take(dev_indices)`, сохраняя schema и vocabulary общего Reader после outer carve.
